    ...
    const result: server.Diagnostic = {
      severity:
        diagnostic.kind === "error"
          ...
          : server.DiagnosticSeverity.Warning,
    ...
    if (result.tooltip !== null) {
      return {
        ...
        range:
          result.range === null
            ? undefined
            : convertRangeFromServer(result.range),
      };
    }
...
connection.onWorkspaceSymbol((request) =>
  reportErrorsFromServer(() => {
    const result = skew.symbolsQuery({
    ...
    return result.symbols.map((symbol) => ({
      ...
      containerName:
        symbol.parent === -1 ? undefined : result.symbols![symbol.parent].name,
    }));
  }),
);
...
connection.onCodeAction((request) =>
  reportErrorsFromServer(() => {
    ...
    if (request.context.diagnostics.length > 0) {
      ...
      if (fixesMap !== undefined) {
        const fixes = fixesMap.get(
          diagnosticKey(request.context.diagnostics[0]),
        );
        if (fixes !== undefined) {
